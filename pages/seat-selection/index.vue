        ...
        <div class="flex flex-row items-center gap-2">
          ...
          <div>
            <h2 class="text-xl font-bold text-gray-800">
              Select Your Seats - {{ currentJourneyType === 'outbound' ? 'Outbound' : 'Return' }} Journey
            </h2>
            <!-- Journey Progress Indicator -->
            <div v-if="hasReturnDate" class="flex items-center gap-2 mt-1">
              <div class="flex items-center gap-1">
                <div :class="currentJourneyType === 'outbound' ? 'bg-red-500' : 'bg-green-500'" class="w-3 h-3 rounded-full"></div>
                <span class="text-sm text-gray-600">Outbound</span>
              </div>
              <div class="w-4 h-px bg-gray-300"></div>
              <div class="flex items-center gap-1">
                <div :class="currentJourneyType === 'return' ? 'bg-red-500' : 'bg-gray-300'" class="w-3 h-3 rounded-full"></div>
      ...
      <div class="bg-white rounded-lg shadow-sm">
        <div class="flex flex-row lg:flex-row">
          ...
          <div class="flex-1 p-6">
            ...
            <!-- Lower Deck -->
            <div class="mb-8">
              <h3 class="text-lg font-semibold text-gray-700 mb-4">Lower Deck</h3>
            ...
            <!-- Upper Deck -->
            <div class="mb-6">
              <h3 class="text-lg font-semibold text-gray-700 mb-4">Upper Deck</h3>
              <div class="bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
                <!-- First row -->
                <div class="flex justify-between mb-3">
                  <div class="flex gap-2">
                    <button
                      v-for="seat in ['1-H', '2-H', '3-H', '4-H', '5-H']"
                      :key="seat"
                      @click="toggleSeat(seat)"
                      :class="getSeatClass(seat)"
                      class="w-12 h-10 border rounded text-xs font-medium transition-colors"
                ...
                <div class="flex justify-between mb-3">
                  <div class="flex gap-2">
                    <button
                      v-for="seat in ['1-F', '2-F', '3-F', '4-F', '5-F']"
                      :key="seat"
                      @click="toggleSeat(seat)"
                      :class="getSeatClass(seat)"
                      class="w-12 h-10 border rounded text-xs font-medium transition-colors"
                ...
                <div class="flex justify-between">
                  <div class="flex gap-2">
                    <button
                      v-for="seat in ['1-E', '2-E', '3-E', '4-E', '5-E']"
                      :key="seat"
                      @click="toggleSeat(seat)"
                      :class="getSeatClass(seat)"
                      class="w-12 h-10 border rounded text-xs font-medium transition-colors"
          ...
          <div class="w-full lg:w-80 bg-gray-50 p-6">
            ...
            <div v-if="hasReturnDate && outboundBooking.seats.length > 0" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h4 class="font-semibold text-green-800 mb-2">✓ Outbound Journey Selected</h4>
            ...
            <div class="mb-6">
              <h3 class="text-lg font-semibold text-gray-800 mb-4">
                {{ currentJourneyType === 'outbound' ? 'Pickup & Drop off' : 'Return Pickup & Drop off' }}
              </h3>
                ...
                <div class="flex items-center justify-between mb-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-gray-800">
                      {{ currentJourneyType === 'outbound' ? (busParams.fromLabel || 'Siem Reap') : (busParams.toLabel || 'Phnom Penh') }}
                    </span>
                  </div>
                  <span class="text-sm font-medium text-gray-600">
                    {{ currentJourneyType === 'outbound' ? (busParams.departureTime || '22:00') : '08:00' }}
                  </span>
                </div>
                <p class="text-sm text-gray-600 ml-5">
                  {{ currentJourneyType === 'outbound' ? 'Behind Old Stadium, Khmer Pub Street, Krong Siem Reap Street' : 'Road 106, Sangkat Doun Penh, Khan Doun Penh' }}
                </p>
              </div>
              ...
              <div class="mb-4">
                <div class="flex items-center justify-between mb-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-gray-800">
                      {{ currentJourneyType === 'outbound' ? (busParams.toLabel || 'Phnom Penh') : (busParams.fromLabel || 'Siem Reap') }}
                    </span>
                  </div>
                  <span class="text-sm font-medium text-gray-600">
                    {{ currentJourneyType === 'outbound' ? (busParams.arrivalTime || '04:00') : '14:30' }}
                  </span>
                </div>
                <p class="text-sm text-gray-600 ml-5">
                  {{ currentJourneyType === 'outbound' ? 'Road 106, Sangkat Doun Penh, Khan Doun Penh' : 'Behind Old Stadium, Khmer Pub Street, Krong Siem Reap Street' }}
                </p>
              </div>
            </div>
            ...
            <div class="mb-6">
              <h3 class="text-lg font-semibold text-gray-800 mb-4">
                {{ currentJourneyType === 'outbound' ? 'Outbound Fare' : 'Return Fare' }}
              </h3>
              <div class="space-y-2">
                ...
                <div v-if="hasReturnDate && outboundBooking.seats.length > 0" class="pt-2 border-t-2 border-gray-300">
                  <div class="flex items-center justify-between">
                    <span class="font-bold text-gray-800">Grand Total</span>
                    <span class="font-bold text-red-500 text-lg">
                      USD {{ (parseFloat(outboundBooking.totalAmount) + totalAmount).toFixed(2) }}
                    </span>
                  </div>
                </div>
              </div>
...
const hasReturnDate = computed(() => busParams.value.returnDate && busParams.value.returnDate !== '')
...
const occupiedSeats = ref(["2-A", "4-A", "1-H", "3-F", "5-E"]);
...
const proceedToBook = () => {
  ...
  if (hasReturnDate.value && currentJourneyType.value === 'outbound') {
    ...
    occupiedSeats.value = ["1-A", "3-B", "2-H", "4-F", "1-E"]
  ...
  if (hasReturnDate.value) {
    ...
    bookingData.grandTotal = (parseFloat(outboundBooking.value.totalAmount) + totalAmount.value).toFixed(2)
  } else {
    // One way booking
